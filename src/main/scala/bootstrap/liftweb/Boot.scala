...
class Boot extends Loggable {
  def boot {
    ...
    LiftRules.addToPackages("code")
		...
		// widgets
		TreeView.init
		LogLevelChanger.init // access from /loglevel/change
		...
		//props file is in /src/main/resources/props/production.default.props
    ...
    // Build SiteMap
    def sitemap = SiteMap(
      Menu.i("Pop Question") / "question" / "create_pop" >> 
				MenuCssClass("pop_window") >>
				If(() => User.loggedIn_? && User.getSession.isDefined, "No class in session") >> 
				LocGroup("main"),
			Menu.i("Pop Question!") / "question" / "pop" >> userOnly >> Hidden,
...
/*      Menu(Loc("Static", Link(List("static"), true, "/static/index"), 
    ...
    SessionMaster.sessionWatchers = SessionInfoDumper :: SessionMaster.sessionWatchers
		...
		DB.addLogFunc {
      case (query, time) => {
         logger.trace("All queries took " + time + "ms: ")
         query.allEntries.foreach({ case DBLogEntry(stmt, duration) =>
            logger.trace(stmt + " took " + duration + "ms")})
            logger.trace("End queries")
      }
    }   
	}
}
